...
from passlib.context import CyptoContext


def get_password_hash(password: str):
    pwd_context = CyptoContext(scheme=['bcrypt'], deprecated='auto')
...
def create_completedtask(db: Session, completedtask: schemas.CreateCompletedTask, user_id: int):
...
def get_completed_tasks(db: Session, user_id: int, skip: int = 0, limit: int = 100):
    return db.query(models.CompletedTask).filter(models.CompletedTask.user_id == user_id).offset(skip).limit(limit).all()
...
def get_completed_task_for_user_query(db: Session, user_id: int, skip: int = 0, limit: int = 100, from_date: datetime = None, to_date: datetime = None):
    query_filter = db.query(models.CompletedTask).filter(models.CompletedTask.user_id == user_id)
    if from_date:
        print(f"From: {from_date}")
        query_filter = query_filter.filter(models.CompletedTask.created_at >= from_date)
    if to_date:
        print(f"To: {to_date}")
